...
class SpectralSpinS(BaseSpinet):
    ...
    def cs_func(self,X,Lap,theta0):
        ...
        def func(theta):
            ...
            return U.sequential_rayleigh_s(y,Lap), (y.t().mm(y)-Y0.t().mm(Y0).detach()).norm()
        return func
    ...
    def learn(self,X,Lap):
        ...
        old_seq_ray = U.sequential_rayleigh_s(Y,Lap)
        ...
        grsmn_grad = self.network.flaten.flatgrad((Y.t().mm(Y)-Y.t().mm(Y).detach()).norm(),retain=True,create=True)
        ...
        new_seq_ray = U.sequential_rayleigh_s(Y2,Lap)
